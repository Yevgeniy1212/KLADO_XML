...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Идентификатор = "ПФ_Проект";
	КомандаПечати.Представление = НСтр("ru = 'Печать проекта'");
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	НужноПечататьМакет = УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПФ_Проект");
	Если НужноПечататьМакет Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"ПФ_Проект",
			НСтр("ru = 'Печать проекта'"),
			ПечатнаяФормаДоговора(МассивОбъектов, ОбъектыПечати),
			,
			"Отчет.ПечатьТекущихЗадач.ПФ_Договор");
	КонецЕсли;
...
Функция ПечатнаяФормаДоговора(МассивОбъектов, ОбъектыПечати)
	...
	|ГДЕ
	|	Задачи.Владелец.Проект В(&Ссылка)
	...
	ШапкаПроект = Пакет[0].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкамСИерархией);
	
	ТаблицаЗадачи = Пакет[1].Выгрузить(ОбходРезультатаЗапроса.ПоГруппировкамСИерархией);
	
	ТабличныйДокумент = Новый ТабличныйДокумент;
	ТабличныйДокумент.КлючПараметровПечати = "ПФ_Проект";
	
	Макет = УправлениеПечатью.МакетПечатнойФормы("Отчет.ПечатьТекущихЗадач.ПФ_Договор");
	
	Пока ШапкаПроект.Следующий() Цикл
		Если ТабличныйДокумент.ВысотаТаблицы > 0 Тогда
			ТабличныйДокумент.ВывестиГоризонтальныйРазделительСтраниц();
		КонецЕсли;
		
		НомерСтрокиНачало = ТабличныйДокумент.ВысотаТаблицы + 1;
		ОбластьЗаголовокОбщий = Макет.ПолучитьОбласть("ЗаголовокОбщий");
		ОбластьЗаголовокОбщий.Параметры.ТекстЗаголовка = "Договоры в рамках проекта """ +ШапкаПроект.Проект + """.";
		ТабличныйДокумент.Вывести(ОбластьЗаголовокОбщий);
		
		Шапка = ШапкаПроект.Выбрать();
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	Представление = "ПР - " + Данные.Наименование;
КонецПроцедуры
