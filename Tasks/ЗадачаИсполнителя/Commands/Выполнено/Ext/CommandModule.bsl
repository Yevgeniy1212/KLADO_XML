...
Процедура ОбработкаКоманды(ПараметрКоманды, ПараметрыВыполненияКоманды)
	...
	Для Каждого Задача Из ПараметрКоманды Цикл
		Если ЭтоЗадачаСогласования(Задача)  Тогда 
			ПоказатьПредупреждение(,НСтр("ru = 'Задача согласования не может быть завершена из списка.'"));
			Возврат;	
		КонецЕсли;
...
&НаСервере
Функция ЭтоЗадачаСогласования(ТекЗадача)
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекЗадача, "ВидЗадачи") = Перечисления.ВидыЗадач.Согласование;	
КонецФункции
