...
Процедура ПриЗаписи(Отказ)
	...
	// Автор: Хохрякова И.В.
	// Дата: 21.07.2021
	Если НЕ ЭтоНовый() И Выполнена
		И ВидЗадачи = Перечисления.видыЗадач.Согласование Тогда
		ОбработатьРезультатСогласования();
	КонецЕсли;
	////////////////////////////////FINISH Кибернетика/////////////////////////////////////// 
...
Процедура ОбработатьРезультатСогласования()
	
	МЗ = РегистрыСведений.ЗапросНаИзменениеРеквизитов.СоздатьМенеджерЗаписи();
	МЗ.Период = ДатаНачала;
	МЗ.Объект = Предмет;
	МЗ.НазваниеРеквизита = НаименованиеРеквизита;
	МЗ.Пользователь = Автор;
	МЗ.Прочитать();
	
	Если РезультатСогласования  Тогда
		ЗадачаОб = Предмет.ПолучитьОбъект();
		ЗадачаОб.ДатаОкончания = МЗ.Значение;
		ЗадачаОб.ДополнительныеСвойства.Вставить("АвторИзменения", Автор);
		ЗадачаОб.Записать();
	КонецЕсли;
	...
	НоваяЗадача.Наименование = "Ознакомится с результатом";
	НоваяЗадача.Автор = Исполнитель;
	...
	ТекстОписание = СтрШаблон("Задача: %1
							|Запрос на изменение даты окончания был %2.",
							НоваяЗадача.ПредметСтрокой,
							?(РезультатСогласования , "согласован", "отменен"));
	НоваяЗадача.Описание = ТекстОписание;
	НоваяЗадача.ВидЗадачи = Перечисления.видыЗадач.Ознакомление;
